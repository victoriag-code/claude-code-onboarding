...
const formatCustomerData = (data) => {
    ...
    let html = `
        ...
        <table style="border-collapse: collapse; width: 100%; max-width: 600px;">
            ...
            <tr style="background: #f5f5f5;">
                <td style="padding: 10px; border: 1px solid #ddd;"><strong>Product Usage Model</strong></td>
                <td style="padding: 10px; border: 1px solid #ddd; ${data.license_type === 'per-user' ? 'color: red; font-weight: bold;' : ''}">${data.license_type === 'per-user' ? 'License' : 'API'}</td>
            </tr>
            <tr>
        ...
        <div style="background: #fff3cd; border: 2px solid #ffc107; padding: 15px; margin-top: 20px; border-radius: 5px;">
            ...
            <p style="color: #856404;">This customer selected the License usage model and requires sales team follow-up to arrange per-user licensing ($200/month per developer).</p>
        </div>
        ` : ''}
...
Product Usage Model: ${data.license_type === 'per-user' ? 'License' : 'API'}
...
This customer selected the License usage model and requires sales team follow-up.
` : ''}
...
app.post('/api/submit-wizard', limiter, async (req, res) => {
    try {
        ...
        const mailOptions = {
            ...
            subject: `[Claude Code Setup] ${customerData.company_name} - ${customerData.license_type === 'per-user' ? 'License Model - SALES REQUIRED' : 'API Model'}`,
            text: text,
            html: html,
